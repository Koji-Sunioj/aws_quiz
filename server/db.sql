...
138	AWS Budgets	Track costs, usage, coverage and utilization for Reserved Instances and Savings Plans. Alerts when cost or usage exceeds drops below budget limit. Annual, quarterly, monthly, daily budgets.	billing
...
110	AWS Organizations	Consolidates multiple AWS accounts into an organization to manage. Available as consolidated billing or all features. Policies can be applied to each account or OU. A Paying account pays for bill, but cannot manipulate resources of others. Linked accounts means all accounts functions are independent.	cloud management
111	AWS Control Tower	Service for creating multi-account environments for compliance and security reasons. Can integrate and control access of other services, account users and enabling encryption on resources.	cloud management
112	AWS Config	Provides resource inventory, auditing capabillities, configuration history, and configuration change tracking on resources against set compliance rules. 	cloud management
